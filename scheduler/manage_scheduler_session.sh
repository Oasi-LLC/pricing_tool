...
# Run from project root: ./scheduler/manage_scheduler_session.sh [start|stop|status|...]
...
case "${1:-status}" in
    start)
        ...
        "$ROOT/scheduler/start_scheduler_session.sh"
        ;;
    stop)
    ...
    status)
        ...
        if screen -list | grep -q "$SESSION_NAME"; then
            ...
            echo "💡 To stop: ./scheduler/manage_scheduler_session.sh stop"
        else
            echo "❌ Status: NOT RUNNING"
            echo ""
            echo "💡 To start: ./scheduler/manage_scheduler_session.sh start"
        fi
        ;;
    logs)
        if screen -list | grep -q "$SESSION_NAME"; then
            ...
            echo "❌ No scheduler session found. Start it first with: ./scheduler/manage_scheduler_session.sh start"
        fi
        ;;
    status-detail)
