...
# Rate limiting: delay (seconds) before each API request to avoid 429
REQUEST_DELAY_SECONDS = 1.2

def _request_with_delay(method, url, **kwargs):
    """Perform request with delay and optional 429 retry."""
...
def get_listing_overrides(listing_id, pms, start_date, end_date):
    ...
    response = _request_with_delay(requests.get, url, headers=headers, params=params)
    data = response.json()
...
def get_daily_data_for_listing(listing_id, pms, start_date, end_date):
    ...
    response = _request_with_delay(requests.post, url, headers=headers, json=payload)
    data = response.json()
...
def generate_pl_daily_for_property(property_key, start_date, end_date):
    ...
    all_records = []
    if use_sequential:
        print("  📌 Sequential mode (heavy property) to avoid rate limits")
        for listing in listings:
            try:
                listing_records = fetch_listing_data(
                    listing, start_date, end_date, all_reservations, pms
                )
...
def generate_pl_daily_for_property_batched(property_key, start_date, end_date):
    ...
    for batch_num, batch_listings in enumerate(batches, 1):
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            # Collect results as they complete (per-request delay is in _request_with_delay)
            for future in concurrent.futures.as_completed(future_to_listing):
                listing = future_to_listing[future]
                try:
                    listing_records = future.result()
